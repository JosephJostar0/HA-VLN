...
@baseline_registry.register_trainer(name="etpnav_trainer")
...
#            #-----Mar/21/26
#            txt_ids=lang_idx_tokens,
#            txt_masks=all_lang_masks,
#            #-----Mar/21/26
#        )
        all_language_features = self.policy.net(
            mode='language',
            #lang_idx_tokens=lang_idx_tokens,
            #lang_masks=all_lang_masks,
            #-----Mar/21/26
    ...
    def train(self) -> None:
        ...
        # --------Mar/21/26
        # --- 强行复活并冻结探路者模块 ---
        self.waypoint_predictor = WaypointPredictionNet(
                observation_space=observation_space,
                model_config=self.config.MODEL
        )
        self.waypoint_predictor.to(self.device)
        self.waypoint_predictor.eval()  # 设为评估模式，不参与梯度更新
        # --------------------------------
        #---------Mar/21/26
