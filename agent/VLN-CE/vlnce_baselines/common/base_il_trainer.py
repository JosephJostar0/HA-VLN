...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _eval_checkpoint( # flag0, 评估时会保存视频
        ...
        new_metric = Calculate_Metric(split=split)
        ...
        while envs.num_envs > 0 and len(stats_episodes) < num_eps:
            ...
            for i in range(envs.num_envs):
                ...
                new_metric(stats_episodes[ep_id], ep_id)
        ...
        for ep_id in stats_episodes.keys():
            keys = stats_episodes[ep_id].keys()
            if 'distance_to_human' in keys or 'collisions_detail' in keys:
                if str(ep_id) not in stats_info.keys():
                    stats_info[ep_id] = {}
                if 'distance_to_human' in keys:
                    stats_info[ep_id]['distance_to_human'] = stats_episodes[ep_id]['distance_to_human']
                    del stats_episodes[ep_id]['distance_to_human']
                if 'collisions_detail' in keys:
                    stats_info[ep_id]['collisions_detail'] = stats_episodes[ep_id]['collisions_detail']
                    del stats_episodes[ep_id]['collisions_detail']
                    
        for k in next(iter(stats_episodes.values())).keys():
            if k == 'collisions':
                aggregated_stats[k] = (
                sum(v[k]['count'] for v in stats_episodes.values()) / num_episodes
                )
            else:
                aggregated_stats[k] = (
        ...
        if config.EVAL.SAVE_RESULTS:
            ...
            if len(stats_info.keys()) != 0:
                info_fname = os.path.join(
                    config.RESULTS_DIR,
                    f"stats_ckpt_{checkpoint_index}_{split}_info.json",
                )
